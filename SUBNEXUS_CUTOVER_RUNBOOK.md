...
5. 功能开关启用属于另一次业务发布，本次部署不启用额外功能。任一异常保留日志和数据现场，只按完成本次门禁后登记的同一 full-release wrapper/run 恢复切换前实际 live；历史旧 SubNexus 仅为连续性 anchor，禁止直接删除新表。当前尚无本次可执行入口。
...
### 15.1 2026-09-08 历史准备阶段快照（合同已由第 15.2 节替代）

以下保留当时的候选值、待办状态和“切换时保留当前 live 为新回滚目标”合同，仅用于审计；其中的待生成项不代表本次发布状态，也不得据此生成本次命令。
...
4. 本轮 `rollback` 删除经身份校验的候选并恢复上述新目标。该次历史草案曾把历史 SubNexus anchor 同时用于 switch、失败恢复和 rollback 门禁；此约束已由第 15.2 节纠正，不得用于当前命令。现行恢复只对新目标的身份与运行合同失败关闭。
...
该准备快照当时尚未生成可执行命令。其候选 SHA、脚本 SHA 或 run 不得作为本次发布输入；本次状态见第 15.4 节。

### 15.2 2026-09-09 retained-live 合同（历史发布设计，原则继续适用）

本次包含用户端视觉性能分级、下拉菜单层级、窄屏日期菜单显示和签到文案。API、请求参数、路由、权限、认证、配置、功能开关、按钮事件及业务处理不变。维护者要求代理完成全部前置工作，最终手动执行切换，并明确要求异常时恢复到切换瞬间正在运行的服务器版本。

| 固定项 | 当前合同与待核验项 |
| --- | --- |
| 本轮一级回滚目标 | 本次全新 `prepare` 时的实际 live；其完整 ID、`.Image`、`.Config.Image`、运行合同和唯一名称必须由新 manifest 实时固定，不能从本表预填 |
| 历史连续性 anchor | 旧 SubNexus ID=`be459424b327ad056ea9bdc02187d6a458fe09082369b354158d6e7f7758beee`，anchor=`/srv/subnexus-migration/cutover/20260905085804-4072165`；只在 prepare 和 switch 提交前校验并继续保留，不是本轮正常 rollback 目标，也不得阻止 switch 开始后的恢复 |
| prepare | 建立全新 run 与 PostgreSQL/catalog/Redis/应用数据备份；manifest 写入 `ui_new_rollback_id/image/config_image/name/state=prepared`。只读固定当前 live，不停止、重命名、重启或创建容器 |
| switch | 只在维护者执行后停止 live 并重命名为 `production-app-ui-prior-<run-id>`；完整复核其 stopped 身份后才创建并启动候选。候选健康且提交成功后继续保留该 stopped 容器，不执行 `docker commit`，也不删除它 |
| 失败恢复与最终 rollback | 提交前故障及 `HUP/INT/TERM/ERR` 自动恢复切换前 live；成功切换后的人工 recover/rollback 精确删除候选并恢复同一 stopped live。此时历史 anchor 缺失或漂移只影响二级灾备证据，不得阻断一级恢复；只有新目标或候选的身份/合同不可信时失败关闭。不默认恢复 PostgreSQL/Redis，不修改 Nginx、设置或功能开关 |
| 已有历史对象 | 空间足够时全部保留。确需清理时仅可按完整 ID/路径/SHA 和无引用证据逐项精确处理；历史 anchor、本轮一级回滚目标、新 run、备份和审计不可删除，禁止 prune、通配符或前缀清理 |
...
本历史合同说明不构成可执行交接命令。旧候选 `bf5aae07bb30b380cb1be154c49149c9c64cc7f7` 当时作为 v0.2.4 发布的切换前 live；该次发布随后已成功，本次实际 live 由第 15.4 节覆盖。所有历史已成功、失败或已回滚的 run 禁止重新用于 prepare/switch。

### 15.3 2026-09-10 v0.2.4 完整发布（已切换，历史交接命令撤回）

本 run=`/srv/subnexus-migration/cutover/20260909171117-2439922` 已由维护者切换成功。此前正式 prepare、全量生产快照 new-old-new 兼容 Gate、never-started probe 和最终审计通过时，manifest 为 `state=prepared/ui_state=prepared/ui_commit_intent=no`；这些字段仅为历史切换前快照，不能作为当前 run 状态。发布 commit/tree=`890828afe0f726abb363029f147e04087fed2bca`/`9fc99f6dac3e8f2028b8f2178ec7ca6ae480d425`，image=`sha256:44e8dcf019338e050756c86aba8d2ecf73390b4d058da2ebf916aba19bea28d9`。当时一级回滚目标为切换前 live `b9de08a4f4134a1a486bfc68537344af564b7f337e2117209a75a9cb3c2fb9f0` / `sha256:db1f6f23238301bbecece8b2e5ff3cccdcca8f09aa87099635e5674ba4877577`，暂存名 `subnexus-cutover-ui-prior-20260909171117-2439922`；它不是本次模型表现监控发布的正常回滚目标。
...
| 固定项 | 已核验值 |
| --- | --- |
| 候选 commit/tree | `33a9601c93304f89a67b7845b4e3b10447edcf96` / `efb0d3a6e9089ecb7eea188f91e5299a3694b8d6` |
| 候选镜像 | `sha256:9342118b00d127deb7fdc1c62fe19a347acf5563a254a3e9541cf309f492506f` |
| 正式 run | `/srv/subnexus-migration/cutover/20260911130706-3232923` |
| 准备时间与状态 | `2026-09-11T13:21:27+00:00`；`state=prepared/ui_state=prepared/ui_commit_intent=no` |
| manifest SHA256 | `8436989b7f3ca1b5c338429a52c3493dae26c688dfed8c71a04a57b31ec63b10` |
| 实际 live / 本次一级回滚容器 | `e389b3b1c4f62fd8d9fb0eb04998b0559d21bf39ae4c1a99bdfc90441def3836`；镜像 `sha256:44e8dcf019338e050756c86aba8d2ecf73390b4d058da2ebf916aba19bea28d9` |
| 切换后保留名称 | `subnexus-cutover-ui-prior-20260911130706-3232923` |
| 新回滚镜像 tag | `subnexus-rollback:model-evaluation-20260911-890828afe0f7` |
| 新回滚镜像归档 | `/srv/subnexus-migration/model-evaluation-20260911/rollback-image.tar`；SHA256=`a33a982c244bbc12d5b62a8200fc408305cc82186e64ec24a3d663f9a4935fb3` |
| 兼容 Gate | `/srv/subnexus-migration/docker-candidate/full-33a9601c9330-compat-8b63adeb67e24bd7/evidence.env`；SHA256=`a813fccfe95616d3dc19755bb89cf14b901d3bc489688192a1e90c6beada729d` |
| 最终审计 | `/srv/subnexus-migration/diagnostics/full-33a9601c9330-final-20260911130706-3232923.evidence`；SHA256=`807c4ab7eaea72a1bad745b4c1a17a99ad209beb5fddaee0f466db1c838c367f`；facts SHA256=`f2367751b502a2d1a735e86f008a949c2ec7cefbcb00bde78560ef912138e722` |

原 controller SHA=`19824a87e3e1de5659cb30664750b71c5c10d374f25bda7f52e6524fe477ee65`、UI library SHA=`8fdfc8253020d61c1e5f0b13b49c340295e0713f552763c66603d32ae949e38d` 保持不变。本次 full wrapper SHA=`7bb384a1700c5c24a90c855f8f444fd6ad01fcae3a21b31b01f4ad3d81cf5c9b`，base 固定为实际线上 `890828afe0f726abb363029f147e04087fed2bca`。

所有备份和 sidecar 已验证；探针从未启动且已精确删除，生产 app/PG/Redis 的身份、启动时间、受保护设置和迁移账本未变化，内部及公网健康/首页均 HTTP 200。新监控开关在准备期缺失/false，切换后仍默认关闭，需要后台配置分组、URL、Key 和模型后再启用。
...
| 固定项 | 已验证值 |
| --- | --- |
| 应用 commit/tree | `ccb69f00132dcb9dcbad76e8c3f542231bb002fd` / `4fe51217a193bed8207e23916277d1329bec2f71` |
| 候选镜像 | `sha256:c8a9db0d90f3c99924d7c53c7b9b41b4b90f26d053341abc254888e58c12df55` |
| 正式 run | `/srv/subnexus-migration/cutover/20260911163046-3338612` |
| 准备时间 | `2026-09-11T16:39:03+00:00` |
| 状态 | `prepared/prepared/no`，RETAINED_READY 有效 |
| manifest SHA256 | `20f1c17d67e37c41debaff189ac09a5ecec6369cc6abcc8fbb199192ecd49600` |
| 当前 live | `b4b66b9ca08f185ecbe2e41fc768ff0f3c2685d9a97a4e51ef095da35aafad24` / `sha256:9342118b00d127deb7fdc1c62fe19a347acf5563a254a3e9541cf309f492506f` |
| 既有回滚目标 | `e389b3b1c4f62fd8d9fb0eb04998b0559d21bf39ae4c1a99bdfc90441def3836` / `sha256:44e8dcf019338e050756c86aba8d2ecf73390b4d058da2ebf916aba19bea28d9` |
| 回滚容器现有名称 | `subnexus-cutover-ui-prior-20260911130706-3232923` |
| retained wrapper SHA256 | `a5b955de3ec51cb1acfc1393290ae2259e8b839fbd3825bda563650e45fd1c3b` |
| 兼容 Gate | `/srv/subnexus-migration/docker-candidate/full-ccb69f00132d-compat-eb3d959d95a343cf/evidence.env` / `e94acd215ff8bd28cb75202948640ac921a8d013227ad366e6689943efe724fc` |
| 验证所用完整快照 SHA256 | `347eff0107d47edd707aadab84f79f11efea240741d3c40dfc16fd67c28f8dfb` |
| 最终 evidence | `/srv/subnexus-migration/diagnostics/full-ccb69f00132d-final-20260911163046-3338612.evidence` / `cc48896506865732eec6f08acb67e2d4d8e1ed6b213674cc11beb9427898a34a` |
| 最终 facts SHA256 | `e44a0e5479aa2ac91c53c05afb24eb2f40e345dca46289e299a691a4ea2e9c16` |

前端 317 文件/2213 测试及 build、完整后端测试、隔离镜像构建、服务器候选 Gate、完整生产副本 new/restart/current-live/new/retained-v0.2.4/new 回归、正式 PostgreSQL/Redis/应用数据备份和 never-started probe 全部通过。完整副本仅新增 9015；原监控任务、加密 Key、HTML 和发布字段跨旧二进制保持。探针从未启动且已精确删除；现网 app/PG/Redis 启动时间、380 条迁移账本及监控设置保持不变，本地及公网 HTTP 200。

本次没有创建 rollback tag、镜像归档或永久回滚容器。用户执行 switch 时，当前 live 暂时保留以便提交前失败恢复；候选健康并提交后精确删除该临时对象。正常人工 rollback 恢复已经存在的 v0.2.4 容器，不自动恢复数据库。既有回滚运行合同已复制到本轮 root-only 证据并按哈希验证；复制的是元数据，不是新增回滚对象。
...
### 15.6 2026-09-16 Grok 视频兼容修复（历史，已由后续发布取代）
...
历史交接命令已撤回，不得重复执行；当前命令见第 15.7 节。

回滚（仅在本次切换后需要恢复时执行）：

历史交接命令已撤回，不得重复执行；当前命令见第 15.7 节。
### 15.6 2026-09-16 Grok 视频兼容修复（历史，已切换）

候选提交 `169469943423b03ae1eebeee218d8436265beb9e`，镜像 `sha256:973742706b5b62e05a3fb848e28faaeb0d43c451257ed4e08825154903286a6a`。prepare run `/srv/subnexus-migration/cutover/20260916042849-1880282`，最终预检通过，状态 `prepared/prepared/no`，尚未切换。现网应用、PostgreSQL、Redis、迁移账本和用户数据未被写入。本次不创建新的回滚对象，沿用既有 `subnexus-cutover-ui-prior-20260915144333-1506840`。
...
### 15.7 2026-09-18 渠道监控 V3（前置完成，待人工切换）


核验时间：2026-09-18 18:25:41 Asia/Shanghai。全部前置完成，状态 `prepared/prepared/no`，未执行正式切换。候选包含国产厂商显示修复、按厂商分类及两级自定义排序。
...
| 核验项 | 值 |
| --- | --- |
| 候选 commit | `d032a91abbb7fb9ae828fd3654095b89ad28b11f` |
| 候选 tree | `7907b9ce57fc87f639db09e3383a012d4cfaae2a` |
| 镜像 | `sha256:adfb848e6c3ff80007946d3aac999b89d6a19030a492607c2763ebcf2c1412f1` |
| 正式 run | `/srv/subnexus-migration/cutover/20260918100818-2913221` |
| manifest SHA256 | `4986f79de115246b2bf5f6ca138f5ebb7db6f126d1bfbc1d23a11dfb50585884` |
| 最终预检 SHA256 | `1830903b8a5717a790ac789af21d96ddd2c98eb8868dd33cbb5baa2094ae585a` |
| PostgreSQL 备份 | `3404653088` bytes，SHA256 `bc38cfaff0717c9b731e8177855ba8cae400aae2fd5ab280f90c486578c0bdc6` |
| 迁移账本 | `384` 条，摘要 `da0fc0f4da2518252ee0db5d9636c45c` |
| 已有回滚容器 | `5b44c72e46bfdeaa5d9bfce967d92f6fb5256e6496efd270a025ff75b16238f5` |
| 已有回滚镜像 | `sha256:10cbbe0c68dd0eef7a701c89f9ed6ef226a8a33f147935c619b58778b044813a` |

前端定向 52 测试、前端构建/类型检查、后端监控定向测试、隔离镜像构建、服务器候选启动/重启/清理、精确镜像 new→current→new→retained→new 兼容验证、备份校验、从未启动的容器配置探针和最终预检通过。国产厂商、顺序持久化及用户分组权限在隔离 API 测试中通过。生产应用、数据库、Redis、既有回滚容器身份/启动时间/重启数均未变，内部和公网健康 200。

本次相对线上没有迁移 SQL 或非测试 repository 源码变化。兼容性验证使用独立合成数据，不等同于重新恢复全量生产快照。准备过程没有对生产业务表执行写入。切换后的实际页面验收由正式发布后完成；备份不构成新增回滚镜像目标。

### 15.8 2026-09-19 Seedance（前置完成，待人工切换）

核验时间：2026-09-19 03:47:48 Asia/Shanghai。根据本目录真实 `handoff-ready.json`，本轮全部发布前置已完成，状态 `prepared/prepared/no`，正式切换由维护者手动执行。采集器和文档生成器均未执行切换或回滚命令。

Seedance 首次发布保持默认关闭。供应商真实出片尚未验通；本次发布准备和兼容验证通过不代表上游视频生成已验收。后续完成真实出片及计费验收后再单独启用。
...
应用回滚不恢复数据库，也不会通过回滚命令覆盖用户计费数据。若后续启用了 Seedance，回滚前必须先关闭新建/上传、停止接收任务，并确认所有在途及 `manual_review` 任务均已结清；存在任何未完成任务时回滚守卫会拒绝继续。切换可能造成短暂连接中断。

| 核验项 | 真实记录 |
| --- | --- |
| 候选 commit | `b52db3527324c0e68046a22a62794ef0c6435a49` |
| 候选 tree | `52b7c3a01dbee1b5361f83c01ec38ab2f57e247a` |
| 候选镜像 | `sha256:c15b4358834796bb74bf75fe1dbbea704b5f7e563466cf290ee7e2813011a85f` |
| 切换前 live commit | `d032a91abbb7fb9ae828fd3654095b89ad28b11f` |
| 正式 run | `/srv/subnexus-migration/cutover/20260918193509-3177062` |
| manifest SHA256 | `202f45ee70a5afe4469f7450d1d22506a0496cae4cdec0e51e31c309df8a398d` |
| 最终审计日志 SHA256 | `8d4fff0e748efb7d0a79766ad90d3dec073f14e2b9eebe79acedb30e1ceb9670` |
| 空库候选 Gate SHA256 | `15c24f4e229d58690297700cacea4300d55000d2d16065c4aa0bd65937ef3520` |
| 新→旧→新兼容 Gate SHA256 | `7174c0c3a2b676c97773ee052e0a4f7d501ca97346d0edaf3ff63c6633f5a1c9` |
| 兼容验证所用备份 SHA256 | `a2530bae9a5816577ec01b27ef0d6f915685f9681aa830627272b270f1072353` |
| 正式 prepare PostgreSQL 备份 | `/srv/subnexus-migration/cutover/20260918193509-3177062/postgresql.dump` |
| 正式 prepare PostgreSQL 备份大小 | `3418269217` bytes |
| 正式 prepare PostgreSQL 备份 SHA256 | `2118434352c551a0b3294efd8767e7929003efab48e0b64988f943f5b16f2cc8` |
| 新回滚镜像归档 SHA256 | `648c2bda8877009567cb496c363a3f91d27b20913c1a71d0c4a661d3a90100a0` |
| 最终审计迁移账本 | `384` 条 / `da0fc0f4da2518252ee0db5d9636c45c` |
| 最终审计本地 / 公网健康 | `200` / `200` |
| 最终审计活动结算 / schema DDL | `0` / `0` |
