...
func TestSettingHandler_GetPublicSettings_ExposesSubNexusMarqueeFlag(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerPublicRepoStub{
		values: map[string]string{
			service.SettingKeySubNexusMarqueeEnabled: "true",
		},
	}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.True(t, resp.Data.SubNexusMarqueeEnabled)
}

func TestSettingHandler_GetPublicSettings_ExposesPaymentRolloutFlags(t *testing.T) {
	gin.SetMode(gin.TestMode)
	repo := &settingHandlerPublicRepoStub{values: map[string]string{
		service.SettingKeySubNexusFirstRechargeEnabled:          "true",
		service.SettingKeySubNexusFirstRechargeConfig:           `{"enabled":true,"price":9.9,"credited_amount":12,"ratio":1.2}`,
		service.SettingKeySubNexusStudentRechargeBenefitEnabled: "true",
		service.SettingKeyStudentRechargeBenefitConfig:          `{"enabled":true,"bonus_rate":0.05,"min_recharge_amount":10,"per_order_cap":100}`,
	}}
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{}), "test-version")
	recorder := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(recorder)
	...
	var resp struct {
		Code int `json:"code"`
		Data struct {
			FirstRechargeEnabled          bool `json:"subnexus_first_recharge_enabled"`
			StudentRechargeBenefitEnabled bool `json:"subnexus_student_recharge_benefit_enabled"`
		} `json:"data"`
	}
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.True(t, resp.Data.FirstRechargeEnabled)
	require.True(t, resp.Data.StudentRechargeBenefitEnabled)
}
