...
-- 三档限额全为 NULL 且累计用量为 0 的行不携带任何可执行的限额，也不参与任何读取，直接删除。
-- 保留存在累计用量的历史行，避免升级时丢失用户用量统计。
-- 幂等：重复执行影响 0 行。
...
DELETE FROM user_platform_quotas
 WHERE daily_limit_usd IS NULL
   ...
   AND COALESCE(daily_usage_usd, 0) = 0
   AND COALESCE(weekly_usage_usd, 0) = 0
   AND COALESCE(monthly_usage_usd, 0) = 0;
