...
>
> 当前权威状态（2026-09-07 Asia/Shanghai）：`F:\Sub2Api\SubNexus` 保留二开用户端界面已迁入当前项目，功能、API、路由、权限、配置和开关继续使用当前实现。候选提交 `f6f6dafe1fb2008d0a6f41dc746ae831babc3b18` 已推送，镜像、Docker Gate、无停机 `prepare`、全新备份、never-started probe 和严格最终审计全部通过；生产仍运行切换前容器 `86104829d490733244c9426a59e82e7a12afa3c590de2fc03f2ccb13344aebbd`，尚未执行本轮 `switch`。只有新 manifest 仍为 `prepared` 且 switch 进程已退出或未运行时，第 13 节旧 Rain run rollback 才提供当前生产应急恢复；switch 进程运行中必须等待返回且不得并发 rollback；进程退出且新 manifest 已离开 `prepared` 后才使用第 14 节同 run rollback。固定旧 SubNexus 回滚对象不变，本轮未创建新的永久回滚对象。当前状态取信于文末最新记录。
