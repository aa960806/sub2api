...
- 当前没有有效 prepared run，禁止执行历史命令。下一步先推送提交并以唯一新文件名安装脚本，再针对已经迁移的当前生产库执行全新无停机 `prepare` 生成新鲜备份；核验后使用 never-started stopped probe 证明新脚本在生产 Docker 上得到相同合同哈希，删除该探针，最终才交付新的维护者单行 `switch` 与对应 `rollback`。旧脚本、历史 run、备份、旧容器和旧镜像继续保留，禁止 prune、Nginx 修改和功能开启。
