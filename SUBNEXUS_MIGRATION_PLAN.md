...
> 状态：最新 `upstream/main` 已合并到独立迁移分支；Batch 1-4、本地夹具、线上只读预检/备份、PostgreSQL 18.4 恢复、真实克隆 migration/adoption、候选关闭态和旧版回归已通过。Redis 8 RDB 实际加载和 Docker 候选仍待完成；通过前禁止生产迁移、候选连接生产库、切流或开启功能
...
线上 PostgreSQL `schema_migrations`/`atlas_schema_revisions`、Redis/存储拓扑和切换前备份证据已经取得；生产 PostgreSQL custom dump 已由 PostgreSQL 18.4 完整恢复到本机隔离库，真实克隆的 migration/adoption、候选关闭态启动和旧版 0.1.135 同库回归均已通过。Release Gate 仍缺 Redis 8 RDB 实际加载和 Docker 候选镜像证据；生产备份中的 Channel Monitor V3 原为开启，切换脚本必须保存旧值后显式关闭，回滚时可恢复。未满足剩余门禁时不得让候选连接生产库、执行生产迁移、打开开关、切流或替换线上版本。
