...
状态：**用户已手动切换成功，USDT BSC/BEP20 已启用**。

切换后核对：线上容器 `cb18fcbf51b194424c97e0fff0174a831c72fb3dc14662fe834dede54b169c58` 使用本次候选镜像，healthy、restart=0，公网健康接口 HTTP 200。发布入口 `check` 通过，BEpusdt Provider 存在、启用且列入支付方式；专属收银台就绪。核对时有 1 笔未处理 USDT 订单，未对其取消、修改或模拟付款。本轮状态核对不代表链上支付已到账。
