<template>
  <article class="group relative z-0 glass-card flex min-h-[286px] flex-col overflow-visible rounded-[24px] p-5 text-left hover:z-20">
    ...
    <div class="mt-5 grid grid-cols-3 gap-2">
      <div class="user-glass-inset monitor-metric-panel rounded-2xl border border-slate-200/80 bg-slate-50/85 p-3 dark:border-dark-700/50 dark:bg-dark-900/40">
    ...
    <ChannelMonitorV3Timeline
      ...
      :end-at="timelineEndAt"
      :bucket-seconds="timelineBucketSeconds"
    />
  </article>
</template>
...
// Cards show the newest completed monitoring bucket, not the range aggregate.
...
<style scoped>
html.dark .monitor-metric-panel {
  background-color: rgb(20 29 46 / 40%);
}
</style>
