    ...
    settings: {
      ...
      features: {
        channelMonitor: {
          ...
          modeV3Hint: '复用 V2 被动聚合数据，使用新版用户端渠道状态界面，不向上游发送探活请求。',
        ...
        affiliate: {
          ...
          signupRewards: {
            title: '注册邀请奖励',
            description: '邀请关系绑定成功后，分别向邀请人和新用户发放一次性余额奖励。独立于充值返利开关，默认关闭。',
            enabled: '启用注册邀请奖励',
            enabledHint: '仅在明确开启后发放奖励；缺失或关闭时不会写入余额和奖励流水。',
            inviterAmount: '邀请人奖励金额',
            inviterAmountHint: '每次有效邀请绑定后发给邀请人的余额，0 表示不发放。',
            inviteeAmount: '新用户奖励金额',
            inviteeAmountHint: '每次有效邀请绑定后发给新用户的余额，0 表示不发放。',
            ipLimit: '启用同 IP 日限额',
            ipLimitHint: '按可信客户端 IP 限制同一 IP 每日可触发的注册奖励次数。',
            ipDailyLimit: '同 IP 每日奖励次数上限',
            ipDailyLimitHint: '仅在启用 IP 限额时生效，必须为大于等于 1 的整数。',
          },
          customUsers: {
            title: '专属用户配置',
      ...
      registration: {
        ...
        ipCooldown: '注册 IP 冷却',
        ipCooldownHint: '防止同一可信客户端 IP 在冷却期内重复创建新账号。',
        ipCooldownSeconds: '冷却秒数',
        ipCooldownSecondsHint: '范围：1-86400 秒。已有账号登录不受影响。',
      ...
      site: {
        ...
        defaultLanguage: '服务端默认语言',
        defaultLanguageHint: '用户未在本地选择语言时使用；留空则跟随浏览器语言。',
        defaultLanguageBrowser: '跟随浏览器',
        defaultLanguageZh: '中文',
        defaultLanguageEn: 'English',
        customerSupport: '客服支持按钮',
        customerSupportHint: '仅在明确启用且 Markdown 内容非空时显示悬浮客服按钮。',
        customerSupportContent: '客服 Markdown 内容',
        customerSupportContentPlaceholder: '填写客服说明、联系方式或 Markdown 二维码图片。',
        customerSupportContentHint: '展示前会清理 HTML；留空则隐藏客服入口。',
        docUrl: '文档链接',
