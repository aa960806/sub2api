...
async function saveConfig() { if (!configValid.value) return; savingConfig.value = true; try { config.admin_notification_emails = notificationEmailsText.value.split(/[\n,;]/).map(value => value.trim()).filter(Boolean); const data = await adminInvoicesAPI.updateConfig({ ...config }); Object.assign(config, data.config); Object.assign(storage, data.storage); configAudits.value = data.config_audits || []; notificationEmailsText.value = config.admin_notification_emails.join('\n'); window.dispatchEvent(new CustomEvent('invoice-config-changed')); appStore.showSuccess(t('invoice.admin.configSaved')) } catch (error: any) { appStore.showError(error?.message || t('common.error')) } finally { savingConfig.value = false } }
...
async function runImmediateAction(action: 'accept' | 'resend'): Promise<boolean> { if (!detail.value) return false; actionLoading.value = true; actionKey ||= createInvoiceIdempotencyKey(); try { if (action === 'accept') await adminInvoicesAPI.accept(detail.value.id, '', actionKey); else await adminInvoicesAPI.resendEmail(detail.value.id, actionKey); actionKey = ''; appStore.showSuccess(t('common.success')); await refreshDetail(); return true } catch (error: any) { appStore.showError(error?.message || t('common.error')); return false } finally { actionLoading.value = false } }
async function runConfirmedAction() { if (confirmAction.value === 'accept' && await runImmediateAction('accept')) confirmAction.value = null }
async function runReasonAction() { if (!detail.value || !reasonAction.value || !reasonText.value.trim()) return; actionLoading.value = true; actionKey ||= createInvoiceIdempotencyKey(); try { const id = detail.value.id, reason = reasonText.value.trim(); if (reasonAction.value === 'release') await adminInvoicesAPI.release(id, reason, actionKey); else if (reasonAction.value === 'reject') await adminInvoicesAPI.reject(id, reason, actionKey); else await adminInvoicesAPI.voidInvoice(id, reason, actionKey); actionKey = ''; reasonAction.value = null; appStore.showSuccess(t('common.success')); await refreshDetail() } catch (error: any) { appStore.showError(error?.message || t('common.error')) } finally { actionLoading.value = false } }
...
async function submitUpload() { if (!detail.value || !selectedFile.value || !uploadValid.value || !uploadMode.value) return; uploading.value = true; try { if (uploadMode.value === 'issue') await adminInvoicesAPI.issue(detail.value.id, { file: selectedFile.value, invoice_date: uploadForm.invoice_date, invoice_code: uploadForm.invoice_code, invoice_number: uploadForm.invoice_number }); else await adminInvoicesAPI.replaceFile(detail.value.id, { file: selectedFile.value, invoice_date: uploadForm.invoice_date, reason: uploadForm.reason.trim() }); uploadMode.value = null; appStore.showSuccess(t('common.success')); await refreshDetail() } catch (error: any) { appStore.showError(error?.message || t('common.error')) } finally { uploading.value = false } }
