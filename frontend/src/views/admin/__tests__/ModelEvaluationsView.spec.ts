...
describe('model evaluation admin controls', () => {
  ...
  it('bounds automatic status checks even if a test stays running', async () => {
    ...
    expect(api.tasks).toHaveBeenCalledTimes(961)
  ...
  it('keeps polling beyond twelve minutes and stops when a long test finishes', async () => {
    vi.useFakeTimers()
    api.tasks.mockResolvedValue({ items: [{ ...task, test_status: 'running' }] })
    const wrapper = render()
    await flushPromises()
    await vi.advanceTimersByTimeAsync(29 * 60 * 1000)
    expect(api.tasks).toHaveBeenCalledTimes(871)
    ...
    await vi.advanceTimersByTimeAsync(2000)
    await flushPromises()
    expect(api.tasks).toHaveBeenCalledTimes(872)
    await vi.advanceTimersByTimeAsync(10000)
    expect(api.tasks).toHaveBeenCalledTimes(872)
    wrapper.unmount()
  })
