...
export interface ParsedWechatResumeRoute {
  orderAmount: number
  orderType: 'balance' | 'subscription' | 'first_recharge_gift'
  paymentType: string
  planId?: number
  openid?: string
...
export function parseWechatResumeRoute(
  ...
  const orderType = rawOrderType === 'first_recharge_gift'
    ? 'first_recharge_gift'
    : rawOrderType === 'subscription' || hasPlanId
      ? 'subscription'
      : 'balance'

  const studentBenefit = readQueryString(query, 'student_benefit').toLowerCase() === 'true'
  if (wechatResumeToken) {
    return {
      ...
      ...(studentBenefit ? { studentBenefit: true } : {}),
    }
  }
...
export function stripWechatResumeQuery(query: LocationQuery): LocationQueryRaw {
  ...
  delete nextQuery.student_benefit
  return nextQuery
}
