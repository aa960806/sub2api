...
if [[ "${1:-}" == --case ]]; then
  ...
  if [[ "$scenario" == anchor_missing ]]; then
    ...
    rm -- "$store/$original.name" "$store/$original.running" "$store/$original.image"
  fi
  fi
  app_id="$current"
  ...
  if [[ "$scenario" == rollback || "$scenario" == rollback_interrupted || "$scenario" == rollback_marker_interrupted || "$scenario" == rollback_remove_response_lost || "$scenario" == rollback_anchor_missing ]]; then
...
if grep -Eq 'ui_new_rollback|new-target' "$subject"; then test_fail 'new rollback target semantics leaked into the fixed-anchor wrapper'; fi
...
for scenario in success stop_failed rename_failed start_failed created_without_id contract_failed unhealthy restore_unhealthy recovery_state_failed signal remove_failed remove_response_lost commit_marker_failed commit_state_failed commit_state_persistent commit_marker_interrupted settings_drift anchor_drift occupied_temporary anchor_missing recover_interrupted rollback rollback_interrupted rollback_without_id rollback_from_recovered rollback_marker_interrupted rollback_remove_response_lost rollback_anchor_missing; do
  ...
  case "$scenario" in
    ...
    rollback_anchor_missing)
      [[ "$(cat "$fixture/containers/$replacement.name")" == production-app && "$(cat "$fixture/containers/$replacement.running")" == true ]] || test_fail 'missing rollback anchor mutated the candidate'
      [[ ! -f "$fixture/containers/$current.name" ]] || test_fail 'missing rollback anchor recreated the removed current'
