...
# Docker reports a requested memory-swap limit as -1 when the daemon's kernel
# does not expose the swap controller (for example, the standard WSL2 kernel).
# The hard memory limit remains enforced in that mode; record the capability so
# the container validator can distinguish this representation from a silently
...
docker_info_warnings="$(docker_rpc info --format '{{json .Warnings}}')" || fail 'cannot inspect Docker daemon warnings'
...
[[ "$(stat -Lc '%d|%i|%u|%a' -- "/proc/${BASHPID:-$$}/fd/9")" == "$evidence_lock_fingerprint" ]] ||
...
on_exit() {
  ...
  if declare -F cleanup_resources >/dev/null 2>&1; then
    if ! cleanup_resources; then
      cleanup_failed='true'
      printf 'ERROR: candidate cleanup did not remove every exact run resource\n' >&2
    fi
  fi
  if declare -F assert_production_unchanged >/dev/null 2>&1; then
    if ! assert_production_unchanged exit_cleanup; then
      cleanup_failed='true'
      printf 'ERROR: production identity check failed after candidate cleanup\n' >&2
    fi
  fi
  ...
  if [[ "$initial_status" -eq 0 && "$gate_result" == 'passed' && "$cleanup_failed" == 'false' ]]; then
    if ! assert_production_unchanged 'before_evidence_publish'; then
      cleanup_failed='true'
      printf 'ERROR: production identity check failed before candidate evidence publication\n' >&2
    fi
  fi
...
write_evidence() {
  ...
  }
  {
    ...
    printf 'docker_info_warnings=%s\n' "$(evidence_value "$docker_info_warnings")"
    printf 'docker_swap_limit_supported=%s\n' "$(evidence_value "$docker_swap_limit_supported")"
...
runtime_cap_add_matches() {
  ...
  for token in "${actual_caps[@]}"; do
    token="${token//\"/}"
    token="${token//[[:space:]]/}"
    # Docker 29 serializes capabilities with the OCI-style CAP_ prefix,
    # while older CLI/API versions emit the bare names. Compare a canonical
    # uppercase, prefix-free form so both representations are equivalent.
    token="${token^^}"
    token="${token#CAP_}"
    [[ -n "$token" ]] || return 1
    found='false'
...
candidate_mounts() {
  local container_id="$1"
  # .RW is a Go bool. Docker 29 renders a bool passed to %s as
  # %!s(bool=true), so use the type-correct verb and compare true/false below.
  docker_rpc inspect --format '{{range .Mounts}}{{printf "%s|%s|%s|%t|%s\n" .Type .Name .Destination .RW .Source}}{{end}}' "$container_id"
}
...
validate_candidate_container() {
  ...
  # Docker's Go-template parser accepts named key/value variables, but not the
  # conventional discard variable `$_` on Docker 29.  Keep the value named
  # even though only the network key is emitted.
  network_names="$(docker_rpc inspect --format '{{range $name, $network := .NetworkSettings.Networks}}{{println $name}}{{end}}' "$container_id")" || return 1
  ...
  [[ "$image_id" == "sha256:$expected_image_id" && "$memory" == "$expected_memory" &&
    "$nano_cpus" == "$expected_nano" && "$pids_limit" == "$expected_pids" ]] || return 1
  if [[ "$docker_swap_limit_supported" == 'true' ]]; then
    [[ "$memory_swap" == "$expected_swap" ]] || return 1
  else
    # With no kernel swap controller Docker reports -1 even when the exact
    # --memory-swap value was requested. The hard memory limit above is still
    # active; reject any other representation rather than silently accepting
    # an unbounded or missing memory policy.
    [[ "$memory_swap" == '-1' ]] || return 1
  fi
  ...
  [[ "$network_count" == '1' && "$network_names" == "$network_name" ]] || return 1
  if [[ "$expected_running" == 'true' ]]; then
    [[ "$network_ids" == "$network_name|$network_id" ]] || return 1
  else
    # A created-but-not-started container has no endpoint yet on Docker 29,
    # so NetworkID is empty. The start-time validation below requires the
    # concrete private network ID once an endpoint exists.
    [[ "$network_ids" == "$network_name|$network_id" || "$network_ids" == "$network_name|" ]] || return 1
  fi
...
validate_network_members() {
  local expected_running="${1:-true}"
  local observed expected
  # Read the network membership map as JSON.  Docker 29 has daemon/CLI
  # combinations where a Go-template map range with two variables fails to
  # parse; JSON keeps the exact container IDs without weakening the set check.
  observed="$(docker_checked candidate_network_members network inspect --format '{{json .Containers}}' "$network_id" |
    python3 -c 'import json, sys
data = json.load(sys.stdin)
if not isinstance(data, dict):
    raise SystemExit(1)
print("\n".join(sorted(str(key) for key in data)))' )" || return 1
  ...
  if [[ "$expected_running" == 'true' ]]; then
    ...
    # Docker 29 does not add created-but-not-started endpoints to the
    # network's Containers map. Older daemons may already list them. The
    # per-container pre-start checks above prove each configured network;
    # reject any partial or foreign membership here.
    if [[ -z "$observed" || "$observed" == "$expected" ]]; then
      return 0
    fi
  fi
  printf 'candidate network member mismatch: observed=%q expected=%q\n' "$observed" "$expected" >&2
  return 1
}
...
create_candidate_containers() {
  ...
  validate_network_members false || fail 'candidate pre-start network members are neither empty nor exactly the three candidate containers'
}
...
candidate_migration_count() {
  local count
  count="$(candidate_pg_psql 'SELECT COUNT(*) FROM schema_migrations;' | tr -d '[:space:]')" || return 1
  [[ "$count" =~ ^[0-9]+$ ]] || return 1
  printf '%s' "$count"
}
...
run_candidate_dependency_smoke() {
  ...
  pg_result="$(candidate_pg_psql 'SELECT 1;' | tr -d '[:space:]')" || fail 'candidate PostgreSQL SELECT 1 failed'
  [[ "$pg_result" == '1' ]] || fail "candidate PostgreSQL returned an unexpected SELECT 1 result: $(printf '%q' "$pg_result")"
  redis_result="$(candidate_redis_cli PING | tr -d '[:space:]')" || fail 'candidate Redis PING failed'
  [[ "$redis_result" == 'PONG' ]] || fail "candidate Redis returned an unexpected PING result: $(printf '%q' "$redis_result")"
}
