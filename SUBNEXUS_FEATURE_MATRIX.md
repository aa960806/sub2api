...
> 最后更新：2026-09-03。当前代码已在 `feature/subnexus-migration` 完成迁移实现和定向/全量测试；隔离 PostgreSQL、miniredis/候选主机 smoke、旧版回滚克隆、线上只读预检及生产备份结构校验已通过。生产备份实际隔离恢复、持久化 Redis 恢复和 Docker 候选仍待完成；通过前不得执行生产迁移、候选连接生产库、切流或开启功能。
