...
| --- | --- | --- | --- | --- | --- | --- | --- |
| F01 | 签到与签到奖励 | `backend/internal/service/activity_service.go`；`backend/internal/handler/activity_handler.go`；迁移 `151_activity_rewards.sql`、`155_activity_checkin_ip.sql`、`163_checkin_frozen.sql`、`184_activity_checkin_streaks.sql`；`frontend/src/components/user/dashboard/UserDashboardCheckIn.vue` | 目标未发现对应活动 service/迁移/路由；已有用户、设置和事务基础可复用 | 按目标 service/repository/handler 重新接入，保留规则和幂等；不复制旧生成文件 | `subnexus_checkin_enabled=false` | 独立活动数据；中风险 | 未开始 |
| F02 | 排行榜 | `activity_service.go` / `activity_handler.go`；`frontend/src/views/user/LeaderboardView.vue`；管理端排行组件 | 目标无 `/leaderboard` 路由或活动排行实现；已有 `usage_logs`、`user_affiliates` 和用量统计服务 | 以上游用量字段为准，只迁移二开展示/规则差异；排行查询不得改变计费，奖励日志唯一约束保证幂等 | `subnexus_leaderboard_enabled=false` | 只读用量优先；中风险 | 未开始 |
| F03 | 活动中心 | `backend/internal/service/activity_center_service.go`、`activity_center_handler.go`、`admin/activity_center_handler.go`；迁移 `152_activity_extensions.sql`、`153_activity_center_items.sql` | 目标无活动中心代码、表或 `/activity-center` 路由 | 独立聚合层；不得覆盖上游已有活动；关闭时不查询活动表 | `subnexus_activity_center_enabled=false` | 活动入口和可见性；中风险 | 未开始 |
