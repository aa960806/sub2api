...
> 最后更新：2026-09-06。当前代码已在 `feature/subnexus-migration` 完成迁移实现和定向/全量测试；隔离 PostgreSQL、Redis 8 RDB、候选主机 smoke、旧版回滚克隆、线上只读预检、生产备份结构校验和 Docker runtime gate 均已通过。首页 Rain + Glass UI 的最终 run `/srv/subnexus-migration/cutover/20260906064219-543291` 已 `READY=prepared`，stopped probe 和最终只读复核通过，尚未 switch。最终人工命令见切换手册第 12 节，额外二开功能仍保持关闭。
