...
> 最后更新：2026-09-03。当前代码已在 `feature/subnexus-migration` 完成迁移实现和定向/全量测试；隔离 PostgreSQL、Redis 8 RDB、候选主机 smoke、旧版回滚克隆、线上只读预检及生产备份结构校验已通过。仅 Docker 候选仍待完成；通过前不得执行生产迁移、候选连接生产库、切流或开启功能。
...
- 当前候选代码门禁、隔离 PostgreSQL、Redis 8 RDB 恢复、miniredis/候选主机 smoke、生产备份克隆和旧版回滚克隆已通过；仅 Docker 镜像运行和维护者最终验收仍待验证；未部署、未执行生产迁移、未连接生产数据库运行候选，Release Gate 尚未通过。
